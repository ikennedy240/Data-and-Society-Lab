...
```{r}
medium_airbnb <- airbnb %>% select(zipcode, price, cleaning_fee, number_of_reviews)
```
...
```{r}
medium_airbnb <- medium_airbnb %>% filter(number_of_reviews>5)
```
...
```{r}
medium_airbnb <- medium_airbnb %>% mutate(
  #clean the price
  clean_price = as.numeric(gsub('\\$|,','',price)), 
  #clean the cleaning fee
  clean_fee = as.numeric(gsub('\\$|,','',cleaning_fee)), 
  #sets cleanfee to zero if it was an NA
  clean_fee = ifelse(is.na(clean_fee),0,clean_fee),
  #make total price the sum of the cleaned variables
  total_price = clean_price+clean_fee)
```
...
```{r}
#take medium airbnb, group by zipcode, calculate the mean price
medium_airbnb %>% group_by(zipcode) %>% summarise(mean_price = mean(total_price))
#notice that I don't assign this to anything, it's just output
```
...
```{r}
# Tidycensus
zip_data <- get_decennial('zcta', variables = c("P0050003","P0050004","P0050006","P0040003","P0010001"), state='NY')
zip_data <- zip_data %>% spread(variable, value) %>% select(zip_code = GEOID, white = "P0050003", black = "P0050004", asian = "P0050006",latinx = "P0040003", full_pop = "P0010001") 
airbnb$zipcode <- as.character(airbnb$zipcode)
airbnb <- inner_join(airbnb, zip_data, by=c('zipcode'='zip_code'))
colnames(airbnb)

write_csv(airbnb, 'airbnb_census.csv')
